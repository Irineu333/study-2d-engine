...
Stack: Kotlin + Skiko (JVM Desktop). Skiko é o backend de render **padrão**, usado por todos os jogos shipped; LWJGL (`:engine-lwjgl`, sobre NanoVG + GLFW + OpenGL 3.3 core) é o **segundo backend ativo**, servindo de sentinela viva do invariante #4 via o entrypoint `runLwjgl` de `:games:demos`.
...
2. **`:engine` não depende de nenhum framework de UI/render.** O módulo `:engine` não declara — direta ou transitivamente — `org.jetbrains.compose.*`, `androidx.compose.*`, `org.jetbrains.skia.*`, `org.jetbrains.skiko.*`, `org.lwjgl.*`, AWT/Swing além do estritamente necessário, nem futuras dependências OpenGL/Vulkan/WebGPU. Quem precisa de qualquer um desses é o módulo de backend correspondente (`:engine-skiko` para Skia; `:engine-lwjgl` para LWJGL).
...
4. **`Renderer`, `Input` e `GameHost` permanecem SPIs obrigatórias.** Skiko é o backend padrão (`:engine-skiko`), usado por todos os jogos shipped (Pong, Demos, Hello-World, Tic Tac Toe, Snake); `:engine` não pode vazar tipos de Skiko (`org.jetbrains.skia.*`, `SkikoView`, `SkiaLayer`) — quem precisa de Skia direto é o `:engine-skiko`. LWJGL é o **segundo backend ativo** (`:engine-lwjgl`, sobre NanoVG + GLFW + OpenGL 3.3 core via `org.lwjgl.*`), exercido pelo entrypoint LWJGL de `:games:demos` (`./gradlew :games:demos:runLwjgl`) como sentinela viva deste invariante. `:engine` também não pode vazar tipos de LWJGL (`org.lwjgl.*`) — quem precisa dessas bindings é o `:engine-lwjgl`. Jogos novos devem usar Skiko por default; o entrypoint LWJGL de Demos é o local canônico para validar o segundo backend.
...
:engine-skiko          ← backend Skiko puro sobre SkiaLayer + JFrame (SkikoRenderer, SkikoInput, SkikoHost) — backend padrão usado por todos os jogos shipped
...
- **Sem dependências escondidas.** Se um módulo precisa de um framework de UI/render (Skiko, AWT/Swing, LWJGL), declara no `build.gradle.kts` daquele módulo. Se `:engine` ganhar uma dependência transitiva proibida, é bug.
