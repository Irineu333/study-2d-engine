...
- `6` Rotated sweep — 16 `CharacterBody2D` retangulares (size 22×10), **cada uma com `transform.rotation` aleatório**, dentro de uma arena axis-aligned. Todo par ball-vs-wall e ball-vs-ball roteia pelo `sweepRotatedRectRotatedRect` (SAT temporal) — sem o `kinematic-rotated-sweep` change, essas bolinhas tunelariam pelas paredes ou congelariam no primeiro contato. F2 mostra os AABBs envelope dos OBBs rotacionados.
- `F1` liga/desliga overlay de FPS (tratado pelo `GameHost`, configurável via `GameConfig.toggleFpsKey`)
- `F2` liga/desliga visualização de colliders (idem, via `GameConfig.toggleCollidersKey`)
