...
- `6` Tumbling swarm — 16 quadrados `CharacterBody2D` com velocidade linear e angular, bouncing elástico contra 4 paredes e entre si. Cada contato roteia pelo caminho rotated do sweep (`sweepRotatedRectRotatedRect`) e aplica resposta de impulso rotacional que muda velocidade linear E angular juntas. F2 mostra os AABBs envelope dos OBBs. As paredes acompanham `tree.size` em tempo real durante resize da janela (via `BoundaryWalls`).
- `F1` liga/desliga overlay de FPS (tratado pelo `GameHost`, configurável via `GameConfig.toggleFpsKey`)
- `F2` liga/desliga visualização de colliders (idem, via `GameConfig.toggleCollidersKey`)
