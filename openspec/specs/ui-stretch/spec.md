# ui-stretch Specification

## Purpose

Lets in-game UI authored against a fixed **design resolution** scale and
letterbox onto the actual surface in step with the gameplay `Camera2D`, instead
of always living in raw screen pixels. `SceneTree` carries a `designSize` and a
`uiStretchMode`, derives a camera-independent **UI stretch transform**, and each
`CanvasLayer` opts in via `followStretch`. The engine's debug screen canvas
stays pixel-locked so tooling is unaffected by the stretch.
