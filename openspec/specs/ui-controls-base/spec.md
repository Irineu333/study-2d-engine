# ui-controls-base Specification

## Purpose

Establishes `Control`, the Godot-style abstract base for in-game UI widgets, on
top of `Node2D`. `Control` carries the UI-layout concerns shared by every widget:
a local rect (`size`), Godot 4-style anchors and offsets with an engine-run
anchor layout pass that resolves each control's `position`/`size` from its parent
rect, layout presets, derived-but-settable `position`/`size` (write-back to
offsets), `visible` for render/hit-test culling of a subtree, and `mouseFilter`
for hit-test participation. It also declares `focusMode`/`focusNeighbor_*`/
`sizeFlags*` as inert fields reserved for the future `ui-focus` and `ui-layout`
changes. `Panel`, `Button`, `Label`, and `ColorRect` extend `Control`; `Circle2D`
stays a plain `Node2D`.
