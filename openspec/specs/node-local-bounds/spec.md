# node-local-bounds Specification

## Purpose

Define a polymorphic local-bounds query on `Node2D` and the world-space and tree-space aggregates derived from it (`worldBounds`, `treeBounds`), plus the math primitives (`Transform.apply`, `Rect.corners`) and shape-level bounds (`Shape2D.localBounds`, `CollisionShape2D.localBounds`) that compose into them. These bounds power marquee selection, zoom-to-fit, group boxes, and rotation-aware UI hit-testing without any node having to manually compose world transforms.
