...
corpos, e os **pontos de contato e normais** que o solver de impulso usou.
Três `WorldDebugWidget` (`ShapeGizmoWidget`, `VelocityGizmoWidget`,
`ContactGizmoWidget`), um buffer de contatos por-`SceneTree` preenchido por
um seam gated no `PhysicsSystem.step` (custo zero quando desabilitado), e o
registro dos três como built-ins togglável no `DebugHud`.
