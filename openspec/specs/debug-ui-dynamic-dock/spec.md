# debug-ui-dynamic-dock Specification

## Purpose

Tornar o dock dos painéis de debug screen-space dinâmico: cada painel carrega um
estado de atribuição explícito (`defaultSlot` + `currentSlot` + `orderInSlot`, ou
flutuante), o arrasto resolve um drop target por região (re-dockar, reordenar dentro
do slot, ou flutuar no miolo) com magnetismo pela janela inteira e pull consistente
além da pilha ocupada, e um indicador de inserção comunica onde o painel cairá.
