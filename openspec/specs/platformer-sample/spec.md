...
O jogo `:games:platformer` é o validador de plataforma da engine: fecha a cadeia textura/sprite/tilemap exercitando `TileMap` (céu e terreno visual), `StaticBody2D` posicionados à mão (chão sólido), e um player `CharacterBody2D` + `AnimatedSprite2D` (gravidade, pulo, andar, flip) dirigido por `scripts/player.lua` via `move_and_collide`. Roda sobre o backend Skiko com scripting Lua.
