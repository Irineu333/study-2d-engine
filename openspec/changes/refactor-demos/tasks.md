...
- [x] 1.3 Criar um helper de overlay de demo (`CanvasLayer` com `Label` título + `Label` descrição + `Button` "← Menu") reutilizável por todas as demos
...
- [x] 2.2 Conectar `pressed` de cada botão para trocar a cena via `addChild`/`removeChild` (guardado contra re-attach), e o `Button` "← Menu" para voltar ao menu
...
- [x] 8.2 Adicionar teste de fumaça: menu monta 5 botões; clicar carrega a demo; "← Menu" volta
