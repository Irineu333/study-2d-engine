...
- **`Sprites & Tiles`** (funde os antigos Sprite + Animated + Tilemap): um `TileMap` monta um chão a partir de um atlas real; um `AnimatedSprite2D` "corre" sobre ele (avanço de frame engine-driven); um `Sprite2D` estático decora a cena com um **item** (uma maçã de frame único) — não um segundo personagem, para a tela ter um só protagonista. O player é um `CharacterBody2D` movendo-se sobre o chão de `StaticBody2D` via `moveAndCollide`. Sentinela cross-backend (Skiko + LWJGL) de `texture-rendering`, `sprite-animation` e `tilemap-visual` numa só tela.
