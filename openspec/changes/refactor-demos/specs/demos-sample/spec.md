...
- **`Transforms`** (funde os antigos Solar system + Scale hierarchy): Sol amarelo central com 8 planetas e luas conhecidas orbitando seus pais; Saturno carrega `SaturnRing`. Exercita composição de transform aninhada em até 4 níveis. Inclui um corpo cujo `scale` oscila (validando `world().scale` → tamanho renderizado do filho, antiga demo Scale) e uma `Camera2D` com **zoom/pan interativo** (scroll/teclas) — primeira cobertura de `Camera2D` entre as demos. O HUD/overlay vive em `CanvasLayer` e NÃO sofre o zoom da câmera.
