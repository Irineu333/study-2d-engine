...
- [x] 8.4 UX: the picker panel was loose text drawn over the scene (no background, hard to read). Restructured `ScenePickerWidget.drawDebug` into a real UI — backed panel (fill + border) sized to content, a `Type "name"` header, a dimmed root→selected breadcrumb, and `Transform (world)` / `Properties` sections with aligned `key  value` rows (immediate-mode, still read-only with zero scene-graph children). Text uses the backend's top-left anchor (no spurious baseline shift) and the last row drops its trailing gap, so the panel insets are uniform. Satisfies the existing "Picker Panel" requirement unchanged.
- [x] 8.5 UX: the panel anchored top-left, colliding with FPS/TimeControls. Re-anchored to the bottom-right corner — the only one free of built-in overlays (HUD top-right, Momentum/Log/Profiler bottom-left) — computing `origin` from the panel's own width/height so the fixed corner stays put as the selection changes.
