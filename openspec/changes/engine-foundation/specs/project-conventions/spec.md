...
The `CLAUDE.md` SHALL describe the project's module layout (`:engine`, `:engine-compose`, `:games:<name>`) and the command to run a game module (`./gradlew :games:<name>:run`). Removal of the template's `:desktopApp` and `:shared` modules MUST be noted.
