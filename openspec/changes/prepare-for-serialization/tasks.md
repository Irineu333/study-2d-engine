...
- [ ] 1.2 Adicionar `org.jetbrains.kotlinx:kotlinx-serialization-json` em `libs.versions.toml` e como `api` dependency em `:engine`
...
- [ ] 2.1 Anotar `Vec2`, `Rect`, `Transform` com `@Serializable`
...
- [ ] 3.1 Implementar auto-suffix em `Node.applyAdd` quando há conflito de nome entre irmãos
- [ ] 3.2 Garantir que auto-suffix aplica também em drenagem de pending adds (`Node.drainPending`)
...
- [ ] 4.1 Criar `com.neoutils.engine.serialization.Signal<T>` com `plusAssign`, `minusAssign`, `emit`
- [ ] 4.2 Implementar `emit` iterando sobre snapshot da lista (tolerar mutação durante emissão)
...
- [ ] 5.1 Criar `com.neoutils.engine.serialization.NodeRef<T : Node>` como `@Serializable` com `path: String`
- [ ] 5.2 Implementar `resolve(from: Node): T?` com walk relativo (`..`, segmentos por `/`, path vazio = self)
...
- [ ] 7.1 `Node` e `Node2D` ganham `@Serializable`. `transform` em `Node2D` vira `@Inspect`. `name` em `Node` vira `@Inspect` (com `@Transient` ou serializável conforme decidido — provavelmente `@Inspect`, faz parte da identidade)
- [ ] 7.2 Marcar campos internos de `Node` (`parent`, `_children`, `isLive`, `scene`, `pendingAdd`, `pendingRemove`) com `@Transient`
...
- [ ] 7.7 `Scene` ganha `@Serializable`; `width`, `height` ficam `@Transient` (vão ser recalculados no `resize`); `input` continua `@Transient`
...
- [ ] 9.6 Garantir que `onEnter` só corre após `Scene.start()` posterior ao load
