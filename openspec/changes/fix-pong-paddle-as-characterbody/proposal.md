...
- **`ball.py` classifica acerto de face vs. quina/edge por geometria** em vez de `abs(n.x) > abs(n.y)`: o bounce angular (english do paddle) só é aplicado quando o centro-y da bola está dentro do vão vertical da **face** do paddle; quina e topo/baixo refletem espelhado na normal. Isso resolve a ambiguidade da normal diagonal na quina (`|n.x| ≈ |n.y|`), que hoje é cara-ou-coroa.
