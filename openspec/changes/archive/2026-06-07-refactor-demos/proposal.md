...
- **Funde 9 + 0 → "Sprites & Tiles"**: `TileMap` (chão) + `AnimatedSprite2D` correndo; player como `CharacterBody2D` sobre `StaticBody2D`. Remove `SpriteDemo`, `AnimatedSpriteDemo`, `TileMapDemo` como slots próprios — o `Sprite2D` estático isolado não ganha decorador dedicado (drawImage já coberto por AnimatedSprite2D/TileMap).
