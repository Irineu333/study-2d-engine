...
class SelectionGizmoWidgetTest {
    ...
    fun `oriented box drawn around the selection matches world apply of corners`() {
        ...
        // Enabling the Inspector is the single switch — the gizmo follows it.
    ...
    fun `nothing drawn without a selection`() {
        ...
        tree.debug.inspector.enabled = true
        // Inspector on but no pick performed → no selection.
        val recorder = RecordingRenderer()
        tree.render(recorder)
    ...
    @Test
    fun `nothing drawn when the inspector is disabled even with a prior selection`() {
        val target = ColorRect().apply {
            name = "Target"
            size = Vec2(40f, 40f)
