...
class SceneRenderCameraTest {
    ...
    fun `with current camera, push wraps draws and pop ends the sequence`() {
        ...
        // Push carries the FIT view transform: scale 1.5, offset (40, 0).
        val push = recorder.events.filterIsInstance<Call.Push>().single()
...
private class RecordingRenderer : Renderer {
    ...
    override fun pushTransform(translation: Vec2, scale: Vec2) { events += Call.Push(translation, scale) }
