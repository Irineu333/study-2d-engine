...
class SceneRenderCameraTest {
    ...
    fun `without current camera, no push or pop is emitted`() {
        ...
        // `ColliderWidget`, the two physics gizmos (`VelocityGizmoWidget`,
        // `ContactGizmoWidget`) and the `SelectionGizmoWidget` — all Node2D,
        // identity — contributing six push/pop pairs to the world pass.
        assertEquals(6, recorder.events.count { it is Call.Push })
        assertEquals(6, recorder.events.count { it is Call.Pop })
    ...
    fun `current camera with degenerate bounds skips view push`() {
        ...
        // `ColliderWidget`, two physics gizmos and `SelectionGizmoWidget`
        // (all Node2D, identity) add six more push pairs. Total: Camera2D +
        // WC + immediate + collider + velocity + contact + selection = 7.
