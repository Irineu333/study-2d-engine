...
A HUD lista uma linha por `DebugWidget` ativo no `tree.debug` registry; clicar uma linha alterna o `enabled` do widget. Cada painel screen-space (`ScreenDebugWidget`) tem um header com controles de janela: o **grip** (grade de pontos) à esquerda arrasta o painel, e à direita ficam **colapsar** (`[_]`, esconde o corpo mantendo só o header) e **fechar** (`[x]`, soft close via `enabled = false` — reabre pela HUD).

O dock é **dinâmico**: arrastar um painel pela barra de título o reposiciona conforme a região onde ele cai. As faixas superior e inferior da tela são fatiadas em três terços (esquerda / centro / direita), formando seis slots de encaixe; soltar o painel sobre uma faixa o **dockа** naquele slot, e soltar no miolo o deixa **flutuante** na posição livre. Dentro de um slot, arrastar acima ou abaixo de outro painel **reordena** a pilha. O magnetismo considera a janela inteira (não só o header): encostar uma borda da janela numa faixa basta para encaixar, e a zona de captura além de uma pilha já ocupada tem sempre ~uma faixa de folga, de modo que o segundo painel encaixa com a mesma margem do primeiro. Durante o arrasto, um **indicador de inserção** mostra o slot e a posição onde o painel cairá (nenhum indicador aparece quando o alvo é flutuar). `BACKSPACE` restaura o layout default: devolve cada painel ao seu `defaultSlot` e expande os colapsados.

Para plugar um gizmo novo num projeto-jogo basta criar uma classe estendendo `ScreenDebugWidget` (overlay 2D em pixels) ou `WorldDebugWidget` (gizmo em coordenadas de mundo, recebe a view transform da `Camera2D` automaticamente) e registrá-la após `tree.start()`:
